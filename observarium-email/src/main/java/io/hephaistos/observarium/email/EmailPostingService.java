 ...
 *
 * <p>Email is inherently fire-and-forget: there is no way to search for an existing
 * "issue" in a recipient's inbox, and there is no thread to comment on. As a result:
 * <ul>
 *   <li>{@link #findDuplicate} always returns {@link DuplicateSearchResult#notFound()}.
 *   <li>{@link #commentOnIssue} always returns a {@link PostingResult#failure} explaining
 *       the limitation.
 * </ul>
 *
 * <p>The email body is formatted as plain text so that it renders readably in any email
 * client without requiring HTML or markdown support.
 */
public class EmailPostingService implements PostingService {
    ...
    /**
     * Always returns {@link DuplicateSearchResult#notFound()} because email has no
     * mechanism for querying previously sent messages.
     */
    @Override
     ...
     *
     * <p>The body is formatted as plain text — not markdown — so that the content is
     * immediately readable in any email client.
     *
     * @return {@link PostingResult#success(String, String) success(null, null)} on
     *         delivery; there is no external issue ID or URL for an email.
     */
    @Override
    ...
    /**
     * Produces a plain-text representation of the event suitable for an email body.
     * All fields are written out explicitly so the message is self-contained.
     */
    private static String buildPlainTextBody(ExceptionEvent event) {
        ...
        sb.append("Message:   ").append(event.message() != null ? event.message() : "(no message)").append("\n");
