 ...
 *
 * <p>Issues are identified across runs by a label derived from the first 12 characters of the event
 * fingerprint: {@code observarium-<hash12>}. The service queries for open issues carrying that
 * label before creating a new one, and adds a comment when a duplicate is found.
 *
...
public class JiraPostingService implements PostingService {
   ...
   *
   * <p>Uses the JQL: {@code project = {projectKey} AND labels = "observarium-{hash12}" AND
   * statusCategory != Done}
   */
  @Override
  public DuplicateSearchResult findDuplicate(ExceptionEvent event) {
    ...
    try {
      ...
      if (!isSuccess(response.statusCode())) {
        log.warn(
            "Jira duplicate search returned HTTP {}: {}", response.statusCode(), response.body());
  ...
  /**
   * Creates a new Jira issue for the given event using ADF (Atlassian Document Format) for the
   * description body. The issue is tagged with both {@code observarium} and the fingerprint-derived
   * label so duplicates can be found in subsequent runs.
   */
  @Override
  public PostingResult createIssue(ExceptionEvent event) {
    ...
    try {
      ...
      if (!isSuccess(response.statusCode())) {
        String msg =
            String.format(
                "Jira createIssue failed with HTTP %d: %s", response.statusCode(), response.body());
        log.error(msg);
  ...
  public PostingResult commentOnIssue(String externalIssueId, ExceptionEvent event) {
    ...
    try {
      ...
      if (!isSuccess(response.statusCode())) {
        String msg =
            String.format(
                "Jira commentOnIssue failed with HTTP %d: %s",
  ...
  private JsonObject buildCreateIssueBody(
      String title, String markdownBody, String fingerprintLabel) {
  ...
  /**
   * Constructs the {@code Authorization: Basic ...} header value by Base64-encoding {@code
   * username:apiToken}.
   */
  ...
  /**
   * Returns the human-readable browse URL for a Jira issue key, e.g. {@code
   * https://mycompany.atlassian.net/browse/OBS-42}.
   */
