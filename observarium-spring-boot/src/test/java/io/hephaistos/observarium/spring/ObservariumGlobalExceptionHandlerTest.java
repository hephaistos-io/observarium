 ...
 *
 * <p>Verifies that {@code handleException} delegates to {@link Observarium#captureException}
 * and then re-throws the original exception unchanged.
 */
...
class ObservariumGlobalExceptionHandlerTest {
    ...
    void handleExceptionCapturesViaObservariumThenRethrows() {
        ObservariumGlobalExceptionHandler handler =
                new ObservariumGlobalExceptionHandler(observarium);

        RuntimeException ex = new RuntimeException("something went wrong");

        assertThatThrownBy(() -> handler.handleException(ex))
                .isSameAs(ex);
    ...
    void handleExceptionRethrowsCheckedExceptionUnchanged() throws Exception {
        ...
        assertThatThrownBy(() -> handler.handleException(checked))
                .isSameAs(checked);
