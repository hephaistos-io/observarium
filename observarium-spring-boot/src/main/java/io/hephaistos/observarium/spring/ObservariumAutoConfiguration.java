 ...
 *
 * <p>Activated when {@code observarium.enabled=true} (or when the property is absent,
 * since {@code matchIfMissing=true}). All beans are marked {@link ConditionalOnMissingBean}
 * so that applications can override any component by declaring their own bean of the same type.
 */
@AutoConfiguration
...
public class ObservariumAutoConfiguration {
    ...
    /**
     * Creates the central {@link Observarium} instance, injecting all {@link PostingService}
     * beans found in the application context (including those registered by the posting service
    ...
    /**
     * Registers the Spring MVC global exception handler as a bean when DispatcherServlet
     * is on the classpath. The {@link ObservariumGlobalExceptionHandler} is a
     * {@link org.springframework.web.bind.annotation.ControllerAdvice} so it must be
     * registered as a bean to be picked up by Spring MVC, not listed in the
