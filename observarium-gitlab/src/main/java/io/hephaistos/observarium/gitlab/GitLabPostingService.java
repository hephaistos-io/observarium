 ...
 *
 * <p>Each distinct exception fingerprint is represented by a label of the form
 * {@code observarium-{first12charsOfFingerprint}}. Duplicate detection searches for
 * open issues carrying that label. Issue creation attaches both the
 * {@code observarium} umbrella label and the fingerprint-specific label so that
 * the project's issue list stays filterable at two granularities.
 */
public class GitLabPostingService implements PostingService {
    ...
    /**
     * Searches for an open GitLab issue that carries the fingerprint label derived
     * from this event. Returns the first match, or {@link DuplicateSearchResult#notFound()}
     * if none exists or if the request fails.
     */
    @Override
    public DuplicateSearchResult findDuplicate(ExceptionEvent event) {
        ...
        try {
            ...
            if (!isSuccess(response.statusCode())) {
                log.warn("GitLab duplicate search returned HTTP {}: {}", response.statusCode(), response.body());
    ...
    /**
     * Creates a new GitLab issue for the given event, labelled with both the
     * {@code observarium} umbrella label and the fingerprint-specific label.
     */
    @Override
    public PostingResult createIssue(ExceptionEvent event) {
        ...
        try {
            HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString());
            if (!isSuccess(response.statusCode())) {
                String msg = "GitLab issue creation returned HTTP " + response.statusCode() + ": " + response.body();
                log.error(msg);
    ...
    /**
     * Posts a comment on an existing GitLab issue identified by its internal issue
     * ID ({@code iid}).
     */
    @Override
    public PostingResult commentOnIssue(String externalIssueId, ExceptionEvent event) {
        ...
        try {
            HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString());
            if (!isSuccess(response.statusCode())) {
                String msg = "GitLab comment creation returned HTTP " + response.statusCode() + ": " + response.body();
                log.error(msg);
                return PostingResult.failure(msg);
            }
