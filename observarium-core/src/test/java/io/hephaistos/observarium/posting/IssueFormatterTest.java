...
class IssueFormatterTest {
    ...
    void title_usesShortClassName_notFullyQualifiedName() {
        ...
        assertTrue(title.contains("SomeException"),
            "Title must include the short class name");
        assertFalse(title.contains("com.example.SomeException"),
            "Title must not include the package prefix");
    }
    ...
    void title_truncatesLongMessage() {
        ...
        assertTrue(title.contains(expectedFragment),
            "Truncated portion must be 77 chars followed by '...'");
    }
    ...
    void title_handlesNullMessage() {
        ...
        String title = IssueFormatter.title(event);
        assertTrue(title.contains("(no message)"), "Null message must produce '(no message)' placeholder");
    }

    @Test
    void title_startsWithObservariumPrefix() {
        ExceptionEvent event = baseEvent().build();
        assertTrue(IssueFormatter.title(event).startsWith("[Observarium]"),
    ...
    void title_exactlyAt80Chars_isNotTruncated() {
        ...
        String title = IssueFormatter.title(event);
        assertTrue(title.contains(msg80),
            "Message of exactly 80 characters must not be truncated");
    }
    ...
    void fingerprintMarker_containsFingerprintValue() {
        String marker = IssueFormatter.fingerprintMarker(FINGERPRINT);
    ...
    void markdownBody_containsFingerprintMarker() {
        ...
        assertTrue(body.contains(IssueFormatter.fingerprintMarker(FINGERPRINT)),
    ...
    void markdownBody_containsExceptionClass() {
        ExceptionEvent event = baseEvent().build();
        assertTrue(IssueFormatter.markdownBody(event).contains("com.example.SomeException"));
    }
    ...
    void markdownBody_containsStackTrace() {
        ExceptionEvent event = baseEvent().build();
        String body = IssueFormatter.markdownBody(event);
        assertTrue(body.contains(event.rawStackTrace()),
            "Markdown body must embed the raw stack trace");
    }
    ...
    void markdownBody_includesTraceId_whenPresent() {
        ...
        String body = IssueFormatter.markdownBody(event);
        assertTrue(body.contains("trace-999"),
            "Markdown body must include the trace ID when present");
    }
    ...
    void markdownBody_omitsTraceId_whenAbsent() {
        ...
        String body = IssueFormatter.markdownBody(event);
        assertFalse(body.contains("Trace ID"),
    ...
    void markdownBody_includesSpanId_whenPresent() {
        ...
        String body = IssueFormatter.markdownBody(event);
        assertTrue(body.contains("span-456"),
            "Markdown body must include the span ID when present");
    }

    @Test
    void markdownBody_includesTags_whenPresent() {
        ExceptionEvent event = baseEvent()
    ...
    void markdownComment_containsTimestamp() {
        ...
        assertTrue(comment.contains(event.timestamp().toString()),
            "Comment must include the event timestamp");
    }
    ...
    void markdownComment_includesTraceId_whenPresent() {
        ...
        String comment = IssueFormatter.markdownComment(event);
        assertTrue(comment.contains("trace-comment-1"),
            "Comment must include trace ID when present");
    }
    ...
    void markdownComment_omitsTraceId_whenAbsent() {
        ExceptionEvent event = baseEvent().traceId(null).build();
        String comment = IssueFormatter.markdownComment(event);
        assertFalse(comment.contains("Trace ID"),
    ...
    void markdownComment_includesTags_whenPresent() {
        ExceptionEvent event = baseEvent()
            .tags(Map.of("request-id", "req-001"))
            .build();
        String comment = IssueFormatter.markdownComment(event);
        assertTrue(comment.contains("req-001"),
            "Comment must include tags when present");
    }

    @Test
    void markdownComment_startsWithOccurredAgainHeader() {
        ExceptionEvent event = baseEvent().build();
        assertTrue(IssueFormatter.markdownComment(event).startsWith("## Occurred Again"),
