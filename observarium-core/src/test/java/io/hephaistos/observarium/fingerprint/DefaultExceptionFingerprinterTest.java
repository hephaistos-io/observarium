...
class DefaultExceptionFingerprinterTest {
    ...
    /**
     * Manufactures a Throwable whose stack trace is fixed and deterministic,
     * independent of the line number where it is created.  We do this by
     * replacing the real stack trace with a hand-crafted one, which also lets
     * us produce two traces that differ only in line numbers.
     */
    private static RuntimeException exceptionWithFixedTrace(String message,
                                                             int lineNumber) {
        RuntimeException ex = new RuntimeException(message);
        ex.setStackTrace(new StackTraceElement[]{
            new StackTraceElement("com.example.Service", "doWork", "Service.java", lineNumber),
            new StackTraceElement("com.example.Controller", "handle", "Controller.java", lineNumber + 10)
        });
        return ex;
    }
    ...
    void sameExceptionClassAndMethods_produceSameFingerprint() {
        ...
        assertEquals(fp1, fp2,
            "Same exception class and same stack frames must produce the same fingerprint");
    }
    ...
    void sameExceptionAtDifferentLineNumbers_produceSameFingerprint() {
        ...
        assertEquals(fp1, fp2,
            "Line number differences must not affect the fingerprint");
    }
    ...
    void exceptionWithCause_fingerprintDiffersFromExceptionWithoutCause() {
        ...
        assertNotEquals(fpNoCause, fpWithCause,
            "Adding a cause to an exception must change the fingerprint");
    }
    ...
    void causeChainIsIncluded_deeperCauseChangesFingerprint() {
        ...
        assertNotEquals(fpA, fpB,
            "Different cause types must produce different fingerprints");
    }
