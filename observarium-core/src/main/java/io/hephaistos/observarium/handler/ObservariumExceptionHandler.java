...
/**
 * A {@link Thread.UncaughtExceptionHandler} that reports fatal uncaught exceptions through
 * {@link Observarium} before the JVM exits.
 *
 * <p>Because the JVM may terminate immediately after invoking this handler,
 * {@link #uncaughtException} blocks for up to {@link #FATAL_WAIT_SECONDS} seconds to give the
 * reporting pipeline time to deliver the event. Install this handler via
 * {@link #install(Observarium)} to preserve any existing default handler as a delegate, or
 * construct it directly when more control is needed.
 */
public class ObservariumExceptionHandler implements Thread.UncaughtExceptionHandler {
    ...
    /**
     * Maximum number of seconds {@link #uncaughtException} will block while waiting for the
     * fatal exception report to be delivered. Kept short to avoid delaying JVM shutdown
     * indefinitely when posting services are unresponsive.
     */
    ...
    /**
     * Creates a handler that reports to the given {@link Observarium} instance and then forwards
     * to an existing handler.
     *
     * @param observarium the Observarium instance to report exceptions through; must not be null
     * @param delegate    an existing handler to invoke after reporting, so that prior behaviour
     *                    (e.g. printing to stderr) is preserved; may be null
     */
    public ObservariumExceptionHandler(Observarium observarium,
     ...
     * completes or {@link #FATAL_WAIT_SECONDS} elapses. Blocking is necessary because the JVM
     * may exit as soon as this method returns, giving the async worker no time to finish.
