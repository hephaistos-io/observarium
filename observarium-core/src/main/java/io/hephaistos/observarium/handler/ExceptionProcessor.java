...
/**
 * Orchestrates the fingerprint -&gt; event build -&gt; scrub -&gt; post pipeline for a single
 * exception.
 *
 * <p>This is an internal component; consumers interact with the library through {@link
 * io.hephaistos.observarium.Observarium} rather than this class directly. Each call to {@link
 * #process} fans out to every configured {@link PostingService}, collecting one {@link
 * PostingResult} per service regardless of individual failures.
 *
 * <p>Instances are safe to share across threads provided the injected collaborators ({@link
 * ExceptionFingerprinter}, {@link DataScrubber}, and each {@link PostingService}) are themselves
 * thread-safe.
 */
public class ExceptionProcessor {
  ...
  /**
   * Runs the full reporting pipeline for the given exception and returns one {@link PostingResult}
   * per configured {@link PostingService}.
   *
   * <p>For each service the pipeline checks for a duplicate issue first. When a duplicate is found
   * the exception is appended as a comment; otherwise a new issue is created. A service failure is
   * captured as a {@link PostingResult#failure} entry so that remaining services are still
   * attempted.
   *
   ...
   * @param tags arbitrary key-value metadata to attach to the event; may be null, which is treated
   *     as an empty map
   ...
   * @param traceId distributed trace ID from the originating thread's MDC context, captured eagerly
  ...
  public List<PostingResult> process(
      ...
      String traceId,
      String spanId) {
    var event = buildEvent(throwable, severity, tags, callerThreadName, traceId, spanId);
